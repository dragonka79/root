...
def device_list():
    ...
    headers = ["Host-Name", "Device Type", "Device ID", "System IP", "Site ID", "Version", "Device Model", "State"]
    t = Texttable(max_width=140)
    t.header(headers)

    for item in items:
        tr = [item['host-name'], item['device-type'], item['uuid'], item['system-ip'], item['site-id'], item['version'], item['device-model'], item['state']]
        t.add_row(tr)
    return(t.draw())
