...
# READ THIS: 
# Don't do this. By that I mean, don't use global variables in a flask application.           
...
def hosts():
    ...
    if request.method == "GET":
        return global_hosts # No need to convert the output to JSON, flask
                            # does it automatically

    elif request.method == "POST": 
        global_hosts = request.get_json() # It changes the JSON data of the 
    # GET request to a python data structure and set that equal to global_hosts
        return {}, 204 # Returning no data, only the HTTP response code

    elif request.method == "PUT": # Create a new resource
        hostname = request.args.get("hostname")
        if not hostname:
            return "must provide hostname on PUT", 400

        host = request.get_json() # Gettig a single host only
        global_hosts[hostname] = host # New entry to the global_host dict, 
